...
tasks.withType<Test> {
    ...
    // CI detection is deliberately NOT configured here. Do not add it back.
    //
    // A forked test worker already inherits the Gradle process environment: the
    // first `environment(...)` call seeds the fork's map from `System.getenv()`
    // before applying the override (Gradle 9.6.1 — the version pinned by
    // gradle/wrapper/gradle-wrapper.properties — DefaultProcessForkOptions:
    // `environment(name, value)` -> `getEnvironment().put(...)`, and
    // `getEnvironment()` lazily calls `getInheritableEnvironment()`, which
    // returns `System.getenv()`; `Test.environment(name, value)` delegates
    // straight there). ProcessBuilderFactory then clears the child env and
    // installs that already-seeded map, so the two LD_LIBRARY_PATH calls above
    // carry a genuine CI/GITHUB_ACTIONS through on a real runner.
    //
    // The previous `environment("CI", System.getenv("CI") ?: "")` (and the same
    // for GITHUB_ACTIONS) therefore added nothing in CI, and locally it
    // *manufactured* a present-but-empty variable. RealRustBridgeProofTest
    // detects CI by presence — `System.getenv("CI") != null` — so a developer
    // machine without the Rust toolchain was forced into strict CI mode and
    // failed instead of taking the documented local fallback. Absence must stay
    // absence; a machine that genuinely exports CI must stay strict.
