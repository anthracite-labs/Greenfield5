...
if [ ! -f "$CORE_DIR/target/release/libgreenfield5_core.so" ] && [ ! -f "$CORE_DIR/target/release/libgreenfield5_core.dylib" ]; then
  ...
  (cd -- "$CORE_DIR" || exit 1; cargo build --release)
fi

printf 'Generating Kotlin bindings...\n'
if [ -f "$CORE_DIR/target/release/libgreenfield5_core.so" ]; then
  (cd -- "$CORE_DIR" || exit 1; cargo run --bin uniffi-bindgen generate --library target/release/libgreenfield5_core.so --language kotlin --out-dir "$ANDROID_JAVA_DIR")
elif [ -f "$CORE_DIR/target/release/libgreenfield5_core.dylib" ]; then
  (cd -- "$CORE_DIR" || exit 1; cargo run --bin uniffi-bindgen generate --library target/release/libgreenfield5_core.dylib --language kotlin --out-dir "$ANDROID_JAVA_DIR")
else
